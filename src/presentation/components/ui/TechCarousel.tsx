...
const TechBadge = memo(({ tech }: TechBadgeProps) => {
  ...
  return (
    ...
    >
      <Icon className={CAROUSEL_CONFIG.iconSize} />
      <span className="text-sm text-text-secondary font-medium whitespace-nowrap">{name}</span>
      {years && (
        <span
          className="text-xs text-text-muted bg-surface-overlay/50 rounded-full"
...
const CarouselRow = memo(({ items, direction, speed = 1 }: CarouselRowProps) => {
  ...
  return (
    <div ref={containerRef} className="overflow-hidden w-full">
      <div ref={innerRef} className="flex" style={{ gap: CAROUSEL_CONFIG.gap }}>
